...
| Jar safety report | `scripts/check-release-jar.sh` via `CI` and `Release Security` workflows | CI and GitHub Release publication | CI artifact and GitHub Release asset | Confirm the known-risk pattern checks passed; inspect the script scope and do not treat a pass as a safety guarantee | implemented |
...
| Exact release-jar runtime smoke | `Release Security` workflow using the already prepared release jar | GitHub Release publication | Actions artifact `release-runtime-smoke-vx.y.z` | Match `plugin_sha256` in both smoke summaries to the release jar entry in `SHA256SUMS.txt`; confirm Paper 1.20.6 and latest stable 1.21 startup, selftest, and clean shutdown | implemented |
