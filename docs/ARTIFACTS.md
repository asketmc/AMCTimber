...
| Runtime security invariant result | `scripts/check-runtime-security.py` in the `CI` workflow | Push to `main`, pull request, manual dispatch | `Check runtime security invariants` CI log | Confirm operation-specific protection, final cancellation/drop ordering, landing authorization, attributed crush damage, and paced item-delivery choke-point checks pass; inspect the focused script scope | implemented |
