...
| SPDX SBOM | `SBOM` and `Release Security` workflows using Syft with checksum-verified download | CI/PR and GitHub Release publication | CI artifact or `sbom.spdx.json` release asset | Confirm checksum/signature on release; inspect with SPDX tooling | implemented |
