...
For the current release tag `v1.0.10`, expect:

- `AMCTimber-1.0.10.jar`
- `SHA256SUMS.txt`
- `sbom.spdx.json`
- `sbom.cdx.json`
...
```bash
cosign verify-blob \
  --bundle AMCTimber-1.0.10.jar.sigstore.json \
  --certificate-identity 'https://github.com/asketmc/AMCTimber/.github/workflows/release.yml@refs/tags/v1.0.10' \
...
```bash
gh attestation verify AMCTimber-1.0.10.jar \
  --repo asketmc/AMCTimber \
  --source-ref refs/tags/v1.0.10
```
